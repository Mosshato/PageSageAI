...
function ClassCard({ cls, onOpen, onDelete }) {
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
          {confirmDelete ? (
            <div onClick={e => e.stopPropagation()} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <span style={{ fontSize: "10px", fontFamily: "monospace", color: "rgba(255,255,255,0.45)" }}>Delete?</span>
              <button onClick={handleDelete} disabled={deleting} style={{ padding: "3px 8px", background: "#ef4444", color: "#fff", border: "none", fontSize: "9px", fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", cursor: deleting ? "not-allowed" : "pointer", borderRadius: "3px", textTransform: "uppercase" }}>
                {deleting ? "..." : "Yes"}
              </button>
              <button onClick={e => { e.stopPropagation(); setConfirmDelete(false); }} style={{ padding: "3px 8px", background: "transparent", color: "rgba(255,255,255,0.4)", border: "1px solid rgba(255,255,255,0.2)", fontSize: "9px", fontFamily: "monospace", fontWeight: 700, cursor: "pointer", borderRadius: "3px", textTransform: "uppercase" }}>
                No
              </button>
            </div>
          ) : (
            <button onClick={e => { e.stopPropagation(); setConfirmDelete(true); }} style={{ padding: "3px 8px", background: "transparent", color: "rgba(239,68,68,0.6)", border: "1px solid rgba(239,68,68,0.3)", fontSize: "9px", fontFamily: "monospace", fontWeight: 700, cursor: "pointer", borderRadius: "3px", textTransform: "uppercase", transition: "all 0.15s ease" }}
      ...
      <div style={{ padding: "14px 16px" }}>
        <div style={{ display: "flex", justifyContent: "space-around", marginBottom: "12px" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: "20px", fontFamily: "monospace" }}>{cls.student_count ?? 0}</div>
            <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "10px", fontFamily: "monospace", letterSpacing: "0.1em" }}>STUDENTS</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ color: (cls.awaiting_grades ?? 0) > 0 ? "#f97316" : "#10b981", fontWeight: 700, fontSize: "20px", fontFamily: "monospace" }}>{cls.awaiting_grades ?? 0}</div>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "16px" }}>
            {classes.map(cls => <ClassCard key={cls.id} cls={cls} onOpen={() => navigate(`/teacher/classes/${cls.id}`)}
              onDelete={async () => {
                await fetch(`${API}/teacher/classes/${cls.id}/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
